import { MenuItem } from "./MenuItem";
import { motion } from "framer-motion";
import profile from "../../assets/profile.jpg";
import { useTranslation } from "react-i18next";
import { BsTelegram, BsGithub, BsLinkedin } from "react-icons/bs";
import { Link } from "react-router-dom";
import resume from "../../assets/resume/Resume.pdf";

export const Header = () => {
  const { t } = useTranslation();

  return (
    <div className="hidden bg-white w-3/12 rounded-2xl lg:flex flex-col items-center justify-between gap-y-2 p-5">
      <div className="flex flex-col items-center gap-y-5">
        <img
          src={profile}
          alt="komeil yeganeh - profile"
          className="rounded-full w-44 h-44 shadow-xl object-cover"
        />
        <h1 className="text-lg md:text-2xl">{t("My name")}</h1>
        <Link to={resume} download target="_blank">
          <motion.button
            className="bg-orange-400 py-2 px-5 rounded-2xl text-sm text-white"
            whileHover={{
              backgroundColor: [
                "rgb(251, 146, 60)",
                "rgba(251, 106, 60)",
                "rgb(251, 146, 60)",
              ],
              color: ["white", "black", "white"],
              transition: {
                duration: 2,
                repeat: Infinity,
              },
            }}
          >
            {t("Resume text")}
          </motion.button>
        </Link>
      </div>
      <div className="mt-2">
        <MenuItem />
      </div>
      <div className="flex gap-x-4 mt-4">
        <a href="https://t.me/komeil261" className="p-2" target="_blank">
          <BsTelegram color="#888" />
        </a>
        <a
          href="https://github.com/komeilyeganeh"
          className="p-2"
          target="_blank"
        >
          <BsGithub color="#888" />
        </a>
        <a
          href="https://www.linkedin.com/in/komeil-yeganeh/"
          className="p-2"
          target="_blank"
        >
          <BsLinkedin color="#888" />
        </a>
      </div>
    </div>
  );
};
